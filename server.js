...
app.get("/", (req, res) =>
  res.sendFile("public/chart.html", { root: path.join(__dirname) })
);
app.get("/users", (req, res) => res.send(users));
app.get("/data", (req, res) => res.send(getData()));
app.get("/dataDistance", (req, res) =>
  res.sendFile("data/distance.json", { root: path.join(__dirname) }, (err) => {
    if (err) res.send("[]");
  })
);
app.get("/update", (req, res) =>
  updaterRSS.run(users, () => updaterDist.run(users, () => res.redirect("/")))
);
app.get("/lastupdate", (req, res) =>
  res.sendFile(
    ...
    (err) => {
      if (err) res.send("1970-01-01T00:00:00Z");
    }
  )
);
