...
module.exports.get = (users) => {
    ...
    users.forEach(user => {
        var userBadges = JSON.parse(fs.readFileSync(`data/badges-${user.username}.json`, 'utf8'));
        ...
        userBadges.forEach(badge => {
            ...
            if (badge.image != 'https://assets.untappd.com/badges/bdg_default_lg.jpg') {
...
*/
    ...
    users.map(user => '<th>' + user.username + ' (' + user.uniquebadges +')</th>').join('') +
    '</tr></thead><tbody>' +
    allBadges.map(badge => {
        return '<tr><td><img src="' + badge.image + '" /> ' + badge.name + (badge.isRetired ? ' (retired)' : '') + '</td>' +
        users.map(user => {
            var userBadge = badge.users.find(b => b.name === user.username);
            if (userBadge) {
                return '<td>' + (userBadge.date ? userBadge.date.toISOString().split('T')[0] : '✓') + (badge.isLevel ? ' (Level ' + (userBadge.level ? userBadge.level : '1') + ')' : '') + '</td>';
            } else {
                return '<td></td>';
            }
        }).join('') +
